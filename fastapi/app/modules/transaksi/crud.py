...
def create_transaksi(db: Session, transaksi: TransaksiCreate):
    new_transaction = Transaksi(nama=transaksi.nama, telfon=transaksi.telfon, alamat=transaksi.alamat, book_date=transaksi.book_date, total_user=transaksi.total_user, description = transaksi.description, status='PENDING', wisata_id = transaksi.wisata_id, total_price = transaksi.total_price)
